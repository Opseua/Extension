...
async function serverRun(inf = {}) {
    ...
    try {
        ...
        if ((await new Promise((resolve) => { chrome.identity.getProfileUserInfo(u => { if (chrome.runtime.lastError) { resolve(null); return; } resolve(u.email || false); }); }))?.includes('opseua@')) {
            ...
            // await new Promise(r => setTimeout(r, (999 * (60 * 1000))));

        }

        // IMPEDIR 'serverRun' DE PROSSEGUIR (A CADA x MINUTOS)
        async function notRun() { await new Promise(r => setTimeout(r, (5 * (60 * 1000)))); await notRun(); }
        if (['NOTE_HP', 'AWS',].includes(gW.devMaster) || isModeIncognito) { logConsole({ e, ee, 'txt': `❌❌❌ IGNORANDO EXECUÇÃO DO server.js ❌❌❌`, }); await notRun(); }

        // RESETAR BADGE
        chromeActions({ e, 'action': 'badge', 'text': '', }); // z_testElementAction({}); return; // TESTES
        ...
        chrome.commands.onCommand.addListener(async function (...inf) {
            try {
                ...
                if (inf?.[0] === 'atalho_1') { commands({ 'type': inf[0], 'origin': 'chrome', }); } else if (inf?.[0] === 'atalho_2') {
                    ...
                    await commandLine({ e, 'command': `${fileProjetos}/Sniffer_Python/src/z_OUTROS/server.vbs TOGGLE_HIDE`, });
        ...
        if ((await new Promise((resolve) => { chrome.identity.getProfileUserInfo(u => { if (chrome.runtime.lastError) { resolve(null); return; } resolve(u.email || false); }); }))?.includes('1@gmail.com')) {
            logConsole({ e, ee, 'txt': `MONITOR DE ABAS ATIVADO`, }); async function enforceTabOrder() {
                let arr = ['try', 'rat', 'ing',], TAB_URL = `https://www.${arr[0]}${arr[1]}${arr[2]}.com/`, B = chrome.tabs, W = chrome.windows, isMoving = false; let moveTab = (tabId, index, retries = 0) => {
